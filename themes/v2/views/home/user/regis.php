...
<script>
    ...
    function ckphone(obj) {
        ...
        if (!reg.test(phone)) {
            $('.phone-panel').after('<span id="nameafter" class="bad" style="margin-left:5px;color:red;font-size:10px">请填写正确的手机号</span>');
        } else {
            // alert(1);
            $.ajax({
                'type':'get',
                ...
                'success':function(e) {
                    if(e.s=='error') 
                        $('.phone-panel').after('<span id="nameafter" class="bad" style="margin-left:5px;color:red;font-size:10px">该手机号已注册</span>');
                    else 
                        sendS(phone);
                }
            });
