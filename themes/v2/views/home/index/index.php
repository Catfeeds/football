    ...
    <?=Tools::u8_title_substr($value->title)?> </a>
               ...
               <?php else:?>
                   <header class="archive-header">
                    ...
                    <?php $album = TkExt::model()->normal()->findAll(['limit'=>4]);?>
                    <?php if($album) foreach ($album as $key => $value) {?>
                        <li <?php if($key%2!=0) {echo 'style="margin-left: 15px"';}  ?>><a href="<?=$this->createUrl('/home/album/info',['id'=>$value->id])?>"><img src="<?=ImageTools::fixImage($value->album?$value->album[0]['url']:'',372,212)?>" alt="">
